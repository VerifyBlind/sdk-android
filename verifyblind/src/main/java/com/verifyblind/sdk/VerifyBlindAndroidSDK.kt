...
class VerifyBlindAndroidSDK(private val config: VerifyBlindConfig) {
    ...
    suspend fun startAuthentication(
        ...
        openAppLink(context, nonce, pkHash, returnUrl)
     ...
     *
     * @param returnUrl İşlem bitince VerifyBlind'ın uygulamanıza geri dönmek için açacağı deeplink
     *        (ör. "verifyblinddemo://callback"). Şeması, partner-portal'da kayıtlı "app return scheme" ile
     *        eşleşmeli; aksi halde VerifyBlind güvenlik gereği geri dönüşü AÇMAZ. null → geri dönüş yok.
     */
    private fun openAppLink(context: Context, nonce: String, pkHash: String, returnUrl: String?) {
        var baseStr = config.verifyblindAppLinkBase.trimEnd('/')
        if (!baseStr.endsWith("/request")) {
            baseStr += "/request"
        }

        val builder = Uri.parse(baseStr)
            .buildUpon()
            .appendQueryParameter("nonce", nonce)
            .appendQueryParameter("pk_hash", pkHash)
        if (!returnUrl.isNullOrBlank()) {
            builder.appendQueryParameter("return", returnUrl)
        }
